...
| 人物面板 / 背包 / 装备 | 行为一致 | 可玩 | 原 `o_body/o_item/o_equip`；使用、装备、卸下、秘籍条件链及存档回归；人物页复用 `v_book` 的 `0x56050058 / 0x56160020 / 0x56160024`，背包复用 `v_item` 的 `0x56160076 / 0x56160033 / 0x56059002` 并保持 853×480 逻辑坐标；CI #862 视觉 E2E 通过 | 已完成主要骨架/素材收口；细部字体间距、人物书页子菜单和部分物品说明动效留到最终视觉验收微调 | [#18](https://github.com/liyh1999/jy3-web-remake/issues/18) |
| 商店 / 经济 | 行为一致 | 可玩 | 原 `o_shop`、`p_order.lua`；买卖、数量、讲价、库存边界、牛家村真实事件回归；商店复用 `v_shop` 的 `0x56160036`，窗口按原 423×336 居中坐标布局，并挂载回 853×480 `#game` 缩放容器；CI #862 视觉 E2E 通过 | 数量输入仍保留 Web 等价交互；原按钮 hover/逐项排版可在最终视觉验收继续微调 | [#18](https://github.com/liyh1999/jy3-web-remake/issues/18) |
| 人物成长 / 队伍 / 武功 / 关系 | 行为一致 | 可玩 | 原 `o_body/o_role/o_teammate/o_skill`；加入/离队、升级、熟练度、关系互动、战斗共享状态、save roundtrip；人物页已进入原 `v_book` 书页视觉骨架 | 属性投影已有固定 snapshot；队伍卡片和武功列表仍是 Web 等价排版，最终视觉验收再做细节对照 | [#24](https://github.com/liyh1999/jy3-web-remake/issues/24) [#18](https://github.com/liyh1999/jy3-web-remake/issues/18) |
