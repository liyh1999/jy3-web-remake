...
上文“当前 Web 差距”记录的是 D2-1 审计时的基线。D2-2 从该基线继续修复；后续实现不回写审计事实本身。


### D2-2：raw gain 最终收口

进一步检索固定上游后确认，不能把第四参数与第三参数绑定：

- 常见：`true,1`
- 常见：`false,100`
- 反例：`c_book.lua` 存在 `true,100`
- 反例：`p_order.lua` 存在 `false,1`

因此第三参数只负责原 `loop/flag`，第四参数独立保留为 `rawVolume`。

固定上游没有 gcore 原生音频实现，也没有可验证的增益曲线文档，所以 Web 层不宣称复原了原生 dB/线性算法。D2-2 采用显式的 `jy3-web-compat-v1` 曲线：

- raw `1` -> Web gain `0.55`
- raw `100` -> Web gain `1.0`
- 1..100 中间值线性插值
- `window.JY_CONFIG.audioGain.raw1/raw100` 可覆盖两个端点
- voice 状态始终保留原 `rawVolume`

这个默认值只用于浏览器听感兼容，不作为“原 gcore 真实曲线”的结论。这样比此前把 `1` 和 `100` 都静默映射为 1.0 更可追踪，也允许后续拿到原运行时实测后只调整配置，不改 Lua。
