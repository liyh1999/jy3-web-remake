(() => {
  ...
  window.JYWeb = {
    ...
    async prepareOriginalWudangHuashan(onProgress) {
      const progress = onProgress || ((message) => { ui.status.textContent = message; });
      const loaded = await window.JYUpstream.prepareWudangHuashanRuntime(progress);
      fengari.load('return __jy_dialogue_enable_original(true)', '@web/enable-original-wudang-huashan-dialogue')();
    ...
    async prepareOriginalGaibangTaohuadao(onProgress) {
      const progress = onProgress || ((message) => { ui.status.textContent = message; });
      const loaded = await window.JYUpstream.prepareGaibangTaohuadaoRuntime(progress);
      fengari.load('return __jy_dialogue_enable_original(true)', '@web/enable-original-gaibang-taohuadao-dialogue')();
      ui.status.textContent = '丐帮/桃花岛原剧情运行时已启用';
      return loaded;
    },
    async prepareOriginalXingxiuXuedaomen(onProgress) {
      const progress = onProgress || ((message) => { ui.status.textContent = message; });
      const loaded = await window.JYUpstream.prepareXingxiuXuedaomenRuntime(progress);
      fengari.load('return __jy_dialogue_enable_original(true)', '@web/enable-original-xingxiu-xuedaomen-dialogue')();
