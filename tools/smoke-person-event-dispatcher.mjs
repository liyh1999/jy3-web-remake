...
G.api['初入少林-慧伦'] = function() shaolin_calls = shaolin_calls + 1 return true end
...
G.api['初入血刀门-血刀老祖'] = function() xuedaomen_calls = xuedaomen_calls + 1 return true end
...
G.trig_event('初入全真-赵志敬')
__jy_story_program_browser_pump(0)
assert(quanzhen_calls == 1, 'original p_person did not dispatch 初入全真-赵志敬')
kind = select(1, __jy_story_program_status('地图系统_人物'))
assert(kind == 'case', 'p_person dispatcher did not keep listening after Quanzhen event')

G.trig_event('初入古墓-小龙女')
__jy_story_program_browser_pump(0)
assert(gumu_calls == 1, 'original p_person did not dispatch 初入古墓-小龙女')
kind = select(1, __jy_story_program_status('地图系统_人物'))
...
G.trig_event('初入少林-慧伦')
__jy_story_program_browser_pump(0)
assert(shaolin_calls == 1, 'original p_person did not dispatch 初入少林-慧伦')
kind = select(1, __jy_story_program_status('地图系统_人物'))
assert(kind == 'case', 'p_person dispatcher did not keep listening after Shaolin event')

G.trig_event('初入峨嵋派-周芷若')
__jy_story_program_browser_pump(0)
assert(emei_calls == 1, 'original p_person did not dispatch 初入峨嵋派-周芷若')
kind = select(1, __jy_story_program_status('地图系统_人物'))
assert(kind == 'case', 'p_person dispatcher did not keep listening after Emei event')

G.trig_event('初入丐帮-洪七公')
__jy_story_program_browser_pump(0)
assert(gaibang_calls == 1, 'original p_person did not dispatch 初入丐帮-洪七公')
kind = select(1, __jy_story_program_status('地图系统_人物'))
assert(kind == 'case', 'p_person dispatcher did not keep listening after Gaibang event')

G.trig_event('初入桃花岛-黄药师')
__jy_story_program_browser_pump(0)
assert(taohuadao_calls == 1, 'original p_person did not dispatch 初入桃花岛-黄药师')
kind = select(1, __jy_story_program_status('地图系统_人物'))
...
G.trig_event('初入血刀门-血刀老祖')
__jy_story_program_browser_pump(0)
assert(xuedaomen_calls == 1, 'original p_person did not dispatch 初入血刀门-血刀老祖')
kind = select(1, __jy_story_program_status('地图系统_人物'))
assert(kind == 'case', 'p_person dispatcher did not keep listening after Xuedaomen event')
...
print('  Xuedaomen NPC events route through the same persistent dispatcher')
`;
